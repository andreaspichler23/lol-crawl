...
def get_player_game_info(api_key, game_id):
    ...
    my_id = get_participant_id(list_identities)
...
def get_player_info(df_matchlist):
    ...
    for game_id in matchlist:
        ...
        dict_player = get_player_game_info(api_key, game_id)
...
def main(summoner_name):
    ...
    # else:
    # df_matchlist = get_match_list()
    # df_matchlist['time'] = pd.to_datetime(df_matchlist['timestamp'], unit='ms') 
    # df_gameinfo = get_player_info( df_matchlist )

    return df_frank, df_beware
